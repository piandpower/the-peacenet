...
namespace docopt {
	inline
	value::value(bool v)
	: kind(Kind::Bool), boolValue(v)
	{
	}

	inline
	value::value(long v)
	: kind(Kind::Long), longValue(v)
	{
	...
	: kind(Kind::String)
	{
		new (&strValue) std::string(std::move(v));
	}
	...
	{
		new (&strList) std::vector<std::string>(std::move(v));
	}
	...
	: kind(other.kind)
	{
		switch (kind) {
			...
			case Kind::StringList:
				new (&strList) std::vector<std::string>(other.strList);
				break;
	...
	: kind(other.kind)
	{
		switch (kind) {
			case Kind::String:
				new (&strValue) std::string(std::move(other.strValue));
				break;

			case Kind::StringList:
				new (&strList) std::vector<std::string>(std::move(other.strList));
				break;
