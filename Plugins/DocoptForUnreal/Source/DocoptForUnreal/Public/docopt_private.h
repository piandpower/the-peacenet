...
namespace docopt {
	...
	class Pattern {
	public:
		virtual bool pattern_is_type(std::string id) const
		{
			return id == "Pattern";
		};
	...
	: public Pattern {
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "LeafPattern" || Pattern::pattern_is_type(id);
		};
		...
		virtual size_t hash() const override {
			FString typeName = TEXT("LeafPattern");
			TArray<TCHAR> typeNameChars = typeName.GetCharArray();

			size_t seed = FCrc::MemCrc32(typeNameChars.GetData(), sizeof(TCHAR) * typeNameChars.Num());
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "BranchPattern" || Pattern::pattern_is_type(id);
		};
		...
		virtual void fix_identities(UniquePatternSet& patterns) {
			for(auto& child : fChildren) {
				...
				if (child.get()->pattern_is_type("BranchPattern"))
				{
					auto bp = static_cast<BranchPattern*>(child.get());
		...
		virtual size_t hash() const override {
			FString typeName = TEXT("BranchPattern");
			TArray<TCHAR> typeNameChars = typeName.GetCharArray();
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Argument" || LeafPattern::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Command" || Argument::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Option" || LeafPattern::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Required" || BranchPattern::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Optional" || BranchPattern::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "OptionsShortcut" || Optional::pattern_is_type(id);
		};
	};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "OneOrMore" || BranchPattern::pattern_is_type(id);
		};
	...
	public:
		virtual bool pattern_is_type(std::string id) const override
		{
			return id == "Either" || BranchPattern::pattern_is_type(id);
		};
...
#pragma mark inline implementations
	...
	static inline std::vector<PatternList> transform(PatternList pattern)
	{
		...
		while(!groups.empty()) {
			...
			auto child_iter = std::find_if(children.begin(), children.end(), [](std::shared_ptr<Pattern> const& p) {
				return p.get()->pattern_is_type("BranchPattern");
			});
			...
			if (child.get()->pattern_is_type("Either")) 
			{
				auto either = static_cast<Either*>(child.get());
			...
			} else if (child.get()->pattern_is_type("OneOrMore")) {
				auto oneOrMore = static_cast<OneOrMore*>(child.get());
				...
				BranchPattern* branch = static_cast<BranchPattern*>(child.get());
	...
	inline void BranchPattern::fix_repeating_arguments()
	{
		...
		for(auto const& group : either) {
			...
			for(auto const& e : group_set) {
				...
				if (!e.get()->pattern_is_type("LeafPattern")) continue;
				auto leaf = static_cast<LeafPattern*>(e.get());

				bool ensureList = false;
				bool ensureInt = false;

				if (leaf->pattern_is_type("Command")) {
					ensureInt = true;
				} else if (leaf->pattern_is_type("Argument")) {
					ensureList = true;
				} else if (leaf->pattern_is_type("Option")) {
					auto o = static_cast<Option*>(leaf);
					if (o->argCount()) {
						ensureList = true;
					} else {
						ensureInt = true;
	...
	inline std::pair<size_t, std::shared_ptr<LeafPattern>> Argument::single_match(PatternList const& left) const
	{
		...
		{
			if(left[i].get()->pattern_is_type("Argument"))
			{
				auto arg = static_cast<Argument const*>(left[i].get());
				ret.first = i;
	...
	inline std::pair<size_t, std::shared_ptr<LeafPattern>> Option::single_match(PatternList const& left) const
	{
		auto thematch = find_if(left.begin(), left.end(), [this](std::shared_ptr<Pattern> const& a) {
			if (!a.get()->pattern_is_type("LeafPattern"))
			{
				return false;
			}
			auto leaf = static_cast<LeafPattern const*>(a.get());
			return this->name() == leaf->name();
		});
		if (thematch == left.end()) {
			return {};
		}
		return { std::distance(left.begin(), thematch), std::static_pointer_cast<LeafPattern>(*thematch) };
	}
